...
BinaryDetectCNN_path = "binary/binary.pth"
BallDetectionCNN_path = "ball_detector/ball_detection.pth"
...
# Define minimal transform for both models (64x64 input expected)
...
def feature_extraction(image_path, binary_model_path = "binary/binary.pth"):
    """
    ...
    ball_model.load_state_dict(torch.load(BallDetectionCNN_path, map_location=device))
    ball_model.eval()
...
def evaluate_dataset(csv_path, image_folder):
    """Evaluate model on a dataset and return results dictionary"""
    # Load ground truth data
    gt_data = pd.read_csv(csv_path, header=None)
    gt_data.columns = ['image_path', 'left_blocked', 'front_blocked', 'right_blocked', 
                      'ball_x', 'ball_y', 'ball_exists']
    
    # Prepare result containers for ball position only
    results = {
        "ball_x": {"preds": [], "labels": []},
        "ball_y": {"preds": [], "labels": []}
    }
    ...
    # Process each image
    for idx, row in tqdm(gt_data.iterrows(), total=len(gt_data), desc="Processing images"):
        ...
        # Only evaluate images where ball_exists=1 in ground truth
        if row['ball_exists'] == 1:
            # Run feature extraction
            predictions = feature_extraction(image_path)
            
            # Store predictions and ground truth only if ground truth has ball
            results["ball_x"]["preds"].append(predictions[4])
            results["ball_y"]["preds"].append(predictions[5])
            results["ball_x"]["labels"].append(row['ball_x'])
            results["ball_y"]["labels"].append(row['ball_y'])
    
    # Calculate metrics
    metrics = calculate_ball_position_metrics(results)
    
    # Print ball position metrics summary
    print_ball_position_summary(metrics)
    
    return metrics

def calculate_ball_position_metrics(results):
    """Calculate metrics for ball position only"""
    metrics = {}
    
    # Regression metrics for ball position
    for feature in ["ball_x", "ball_y"]:
        preds = results[feature]["preds"]
        labels = results[feature]["labels"]
        
        if preds:  # Only calculate if we have predictions
            mse = mean_squared_error(labels, preds)
            r2 = r2_score(labels, preds) if len(preds) > 1 else 0
...
def print_ball_position_summary(metrics):
    """Print summary of ball position metrics"""
    print("\nBall Position Metrics (only for images with ball_exists=1 in ground truth):")
    
    for feature in ["ball_x", "ball_y"]:
        if metrics[feature]["mse"] is not None:
            print(f"  {feature.replace('_', ' ').title()}:")
            print(f"    MSE: {metrics[feature]['mse']:.4f}")
            print(f"    R²: {metrics[feature]['r2']:.4f}")
            print(f"    Number of samples: {metrics[feature]['count']}")
        else:
            print(f"  {feature.replace('_', ' ').title()}: No valid predictions")

def main():
    print("\nEvaluating on test dataset...")
    evaluate_dataset(test_data, test_image_folder)
