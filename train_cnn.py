...
from cnn import DetectCNN
from cnn_adv import HybridMobileNetV2
from feature_extractor import FeatureExtractor


class CustomImageDataset(Dataset):
    def __init__(self, model_name, dataset_dir, env_name, size, transform=None):
        self.model_name = model_name
        self.env_name = env_name
        csv_file = f'{dataset_dir}/{env_name}_{model_n}_{str(size)}.csv'
        print("labels are at: ", csv_file)
        if size == 10000:
            self.image_folder = f'{dataset_dir}/images'
        else:
            self.image_folder = f'{dataset_dir}/test_images'
    ...
    def __getitem__(self, idx):
        if self.model_name == "single_frame":
            ...
            image_name1 = self.data.iloc[idx, 0]
            image_name2 = self.data.iloc[idx, 1]
            if self.env_name == "skiing":
                label = float(self.data.iloc[idx, -1])
            else:
                label = self.data.iloc[idx, 2:].values.astype(float)

            image_path1 = os.path.join(self.image_folder, image_name1)
            image_path2 = os.path.join(self.image_folder, image_name2)
            ...
            image = np.stack([img1, img2], axis=0)  # shape: (2, 3, H, W)

            return torch.tensor(image, dtype=torch.float), torch.tensor(label, dtype=torch.float)


def compute_loss(pred, target, env_name, model_name):
    ...
    if env_name == "boxing":
        if model_name == "single_frame":
            binary_target = torch.clamp(target[:, [0, 3, 4, 7]], 0, 1)
            loss_binary = binary_loss(pred[:, [0, 3, 4, 7]], binary_target)
            loss_float = float_loss(torch.cat((pred[:, 1:3], pred[:, 5:7]), dim=1),
                                    torch.cat((target[:, 1:3], target[:, 5:7]), dim=1))
            total_loss = loss_binary + loss_float
        else:
            binary_target = torch.clamp(target[:, [0, 3, 6, 9]], 0, 1)
            loss_binary = binary_loss(pred[:, [0, 3, 6, 9]], binary_target)
            loss_float = float_loss(torch.cat((pred[:, 1:3], pred[:, 4:6], pred[:, 7:9], pred[:, 10:]), dim=1),
                                    torch.cat((target[:, 1:3], target[:, 4:6], target[:, 7:9], target[:, 10:]), dim=1))
            total_loss = loss_binary + loss_float

    elif env_name == "redball":
...
def evaluate_batch(pred, target, env_name, model_name):
    ...
    if env_name == "boxing":
        for i in range(n_features):
            if model_name == "single_frame":
                if i in [0, 3, 4, 7]:  # Binary features
                ...
                for i in range(n_features):
                    if i in [0, 3, 6, 9]:  # Binary features for boxing
                        ...
                        # For continuous features, calculate standard deviation of differences
                        diff = predictions[:, i] - targets[:, i]
                        std_dev = np.std(diff)
                        feature_metrics.append(("std_dev", std_dev))
...
def evaluate_model(model, data_loader, device, env_name, model_name):
    model.eval()
    total_loss = 0.0
    predictions = []
    targets = []
    ...
    with torch.no_grad():
        for images, labels_true in data_loader:
            if env_n == "skiing" and model_name == "double_frame":
                images, labels_true = images.to(device), labels_true.unsqueeze(dim=1).to(device)
            else:
            ...
            if model_name == "single_frame":
                ...
                if env_n == "skiing":
                    labels_pred = model(images[:, 0], images[:, 1], train_flag=True)
                else:
                    labels_pred = model(images[:, 0], images[:, 1])
            loss = compute_loss(labels_pred, labels_true, env_name, model_n)
    ...
    if env_name == "boxing":
        if model_name == "single_frame":
            for i in range(n_features):
                if i in [0, 3, 4, 7]:  # Binary features for boxing
                    # Convert predictions to binary (0 or 1)
                    pred_binary = (predictions[:, i] > 0.5).astype(int)
                    target_binary = (targets[:, i] > 0.5).astype(int)
                    accuracy = (pred_binary == target_binary).mean()
...
env_n = "skiing"  # redball, skiing, lb_foraging, mario
model_n = "double_frame"  # single_frame, double_frame
backbone = "scratch"  # mobilenet, efficientnet, scratch

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
num_epochs_single_frame = 300
num_epochs_double_frame = 100
batch_size = 128
learning_rate = 0.0001
current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
log_path = os.path.join('./logs/obj_detection', env_n, model_n, backbone + "_" + current_time)
os.makedirs(log_path, exist_ok=True)

if __name__ == "__main__":
    # Dataset initialization
    dataset_dir = f'./obj_detection/dataset/{env_n}'
    train_valid_dataset = CustomImageDataset(model_n, dataset_dir, env_n, size=10000
        # csv_file=f'{dataset_dir}/{env_n}_{model_n}_{str(10000)}.csv',
        # image_file=f'{dataset_dir}/{env_n}_{model_n}_{str(10000)}.npy'
    )
    test_dataset = CustomImageDataset(model_n, dataset_dir, env_n, size=2000
        # csv_file=f'{dataset_dir}/{env_n}_{model_n}_{str(2000)}.csv',
    ...
    if model_n == "single_frame":
        num_epochs = num_epochs_single_frame
        if backbone == "scratch":
            model = DetectCNN(env_n, num_channels, image_height, image_width).to(device)
        else:
            model = HybridMobileNetV2(env_n, backbone, freeze_backbone=True).to(device)
    elif model_n == "double_frame":
        num_epochs = num_epochs_double_frame
        model = FeatureExtractor(env_n, backbone, freeze_cnn=True).to(device)
    else:
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Progress"):
        ...
        for images, labels_true in train_pbar:
            if env_n == "skiing" and model_n == "double_frame":
                images, labels_true = images.to(device), labels_true.unsqueeze(dim=1).to(device)
            else:
                images, labels_true = images.to(device), labels_true.to(device)
            if model_n == "single_frame":
                ...
                if env_n == "skiing":
                    ...
                    labels_pred = model(images[:, 0], images[:, 1])
            loss = compute_loss(labels_pred, labels_true, env_n, model_n)
            ...
            batch_metrics = evaluate_batch(labels_pred, labels_true, env_n, model_n)
        ...
        # Validation phase
        val_loss, val_mse_per_feature, val_feature_metrics, val_r2_scores = evaluate_model(model, val_loader, device, env_n, model_n)
    ...
    test_loss, test_mse_per_feature, feature_metrics, r2_scores = evaluate_model(model, test_loader, device, env_n, model_n)
